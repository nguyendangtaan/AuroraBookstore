...
class AuthorController extends Controller
{
    ...
    public function store(AddAuthorRequest $request)
    {
        if($request->validated()){
            $data = $request->all();
            $data['author_img'] = $this->saveImage($request->file('author_img'));
    ...
    public function update(UpdateAuthorRequest $request, Author $author)
    {
        if($request->validated()){
            ...
            if($request->has('author_img')){
                $this->removeProductImageFromStorage($author->thumnail);
                $data['author_img'] = $this->saveImage($request->file('author_img'));
            }
    ...
    {
        $this->removeProductImageFromStorage($author->author_img);
